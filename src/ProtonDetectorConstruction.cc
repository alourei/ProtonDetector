...
ProtonDetectorConstruction::ProtonDetectorConstruction() {
	 ...
	 radiusGasTube = 85*mm;
	 lengthGasTube = 300*mm;

	 degraderThickness = 4.65*mm;
	 degraderAngle = 0*deg;
...
void ProtonDetectorConstruction::DefineMaterials(){
	  ...
	  //Air
	   ...
	   density = PressureFactor*0.0054586*g/cm3;	   pressure = PressureFactor*atmosphere;
	   ...
	   //PressureFactor = 1.05263;  // 800 torr
	    PressureFactor = 1;  // 800 torr
	   density = PressureFactor*0.0016807*g/cm3;
	   pressure = PressureFactor*atmosphere;
	   G4Material *ArisoButane = new G4Material("ArIso-2.5%", density, nel=2, kStateGas,
	 			             	 	 	 	  temperature= 293.15*kelvin, pressure);
	   ArisoButane->AddMaterial(Ar, 0.975);
	   ArisoButane->AddMaterial(isobutane, 0.025);
...
G4VPhysicalVolume* ProtonDetectorConstruction::ConstructDetector(){
	...
	if(detectorGeometry == "box"){
		...
		G4double gasChamberPart2_Radius0 = (85.0/2-3.)*mm;
		G4double gasChamberPart2_Radius = (85.0/2.)*mm;
		...
		gasTube_Length = (lengthGasTube)/2.*mm;
		StartAngle = 0.*deg;
		SpanAngle = 360.*deg;
		...
		//pttoMaterial= G4Material::GetMaterial("ArMethane");
		pttoMaterial= G4Material::GetMaterial("ArIso-2.5%");

		G4cout<<"Selected Gas"<<pttoMaterial->GetName()<<G4endl;

		G4VisAttributes *gasBoxVisAtt = new G4VisAttributes(G4Colour(1.0,1.0,1.0));
		...
		G4ThreeVector positionWindowFront = G4ThreeVector(0.0,0.0,-2*cm-(gasTube_Length-halfHeightWin)); // w.r.t. Ion Chamber
